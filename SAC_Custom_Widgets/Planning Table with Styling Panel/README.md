...
**Re-upload JSON + ZIP** (widget version **1.3.9**). The toolbar must show **v1.3.9**. **Builder is native SAC** (same as 1.3.2): Rows, Columns with Measures nested, Filters, and the model dimension/measure lists. A custom Builder was what removed those pickers.
