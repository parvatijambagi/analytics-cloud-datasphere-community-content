...
// SAP's own fiscal-native hierarchy attributes (FISCAL_CALPERIOD /
// FISCAL_CALQUARTER / FISCAL_CALYEAR) name the fiscal year by the year it
// STARTS, not the year it ends -- unlike this widget's own FY convention.
// A raw code like "202501" here means SAP FY2025 (Oct 2025 - Sep 2026)
// Period 01, i.e. Oct 2025, which this widget calls FY2026 P01. If this
// were misread as a plain calendar year+month it would resolve to
// January 2025 instead, silently shifting every period by several months
// and years -- exactly the "shows 2027 instead of 2026" symptom reported
// in production.
const fiscalCalPeriodMember = api.parseLooseDate('[Date].[FYQP].[Date.FISCAL_CALPERIOD].[202501]')
if (!fiscalCalPeriodMember || api.fiscalYearOf(fiscalCalPeriodMember) !== 2026 || api.fiscalPeriodOf(fiscalCalPeriodMember) !== 1) {
  throw new Error('FISCAL_CALPERIOD 202501 should resolve to this widget\'s FY2026 P01 (Oct 2025), got ' +
    (fiscalCalPeriodMember && (api.fiscalYearOf(fiscalCalPeriodMember) + ' P' + api.fiscalPeriodOf(fiscalCalPeriodMember))))
}
const fiscalCalPeriodMemberP10 = api.parseLooseDate('[Date].[FYQP].[Date.FISCAL_CALPERIOD].[202510]')
if (!fiscalCalPeriodMemberP10 || api.fiscalYearOf(fiscalCalPeriodMemberP10) !== 2026 || api.fiscalPeriodOf(fiscalCalPeriodMemberP10) !== 10) {
  throw new Error('FISCAL_CALPERIOD 202510 should resolve to FY2026 P10, got ' +
    (fiscalCalPeriodMemberP10 && (api.fiscalYearOf(fiscalCalPeriodMemberP10) + ' P' + api.fiscalPeriodOf(fiscalCalPeriodMemberP10))))
}
const fiscalCalQuarterMember = api.parseLooseDate('[Date].[FYQP].[Date.FISCAL_CALQUARTER].[20251]')
if (!fiscalCalQuarterMember || api.fiscalYearOf(fiscalCalQuarterMember) !== 2026 || api.fiscalPeriodOf(fiscalCalQuarterMember) !== 1) {
  throw new Error('FISCAL_CALQUARTER 20251 (Q1) should resolve to FY2026 P01 (Oct 2025), got ' +
    (fiscalCalQuarterMember && (api.fiscalYearOf(fiscalCalQuarterMember) + ' P' + api.fiscalPeriodOf(fiscalCalQuarterMember))))
}
// A plain calendar-hierarchy code (no FISCAL_CAL* attribute name) must
// still be read as a literal calendar year+month, unchanged from before.
const calendarStyleMember = api.parseLooseDate('[Date].[YQM].&[202501]')
if (!calendarStyleMember || calendarStyleMember.getFullYear() !== 2025 || calendarStyleMember.getMonth() !== 0) {
  throw new Error('Non-fiscal calendar code 202501 must still parse as literal Jan 2025')
}
